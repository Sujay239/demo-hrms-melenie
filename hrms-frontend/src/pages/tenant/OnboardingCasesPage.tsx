...
export const OnboardingCasesPage: React.FC = () => {
  ...
  const [rejectReason, setRejectReason] = useState(
    "Verification documentation submitted is illegible or failed compliance check.",
  );
  ...
  const handleApprove = (caseItem: OnboardingCase) => {
    setIsApproving(true);
    setTimeout(() => {
      const updated = mockStorage.approveOnboardingCase(
        caseItem.id,
        currentUser.name,
      );
      ...
      if (updated) {
        toast.success(
          `🎉 ${caseItem.candidateName} approved! Successfully converted to permanent active employee.`,
        );
        reloadCases();
      } else {
  ...
  const handleReject = (caseItem: OnboardingCase) => {
    setIsRejecting(true);
    setTimeout(() => {
      const updated = mockStorage.rejectOnboardingCase(
        caseItem.id,
        rejectReason,
        currentUser.name,
      );
      ...
      if (updated) {
        toast.error(
          `Onboarding for ${caseItem.candidateName} has been rejected & access revoked.`,
        );
        reloadCases();
      } else {
  ...
  const readyForReview = cases.filter(
    (c) => c.status === "SUBMITTED_FOR_REVIEW",
  ).length;
      ...
      <div className="space-y-6 animate-in fade-in duration-200 w-full max-w-full">
          ...
          <div className="flex items-center gap-3">
            ...
            {isApproved ? (
              <Badge variant="emerald" size="md">
                <CheckCircle2 className="w-3.5 h-3.5 mr-1" /> PERMANENT EMPLOYEE
                ACTIVE
              </Badge>
            ) : isRejected ? (
              <Badge variant="rose" size="md">
                <XCircle className="w-3.5 h-3.5 mr-1" /> ACCESS REVOKED &
                REJECTED
              </Badge>
            ) : isReady ? (
          ...
          <div className="p-4 bg-rose-50 border-2 border-rose-300 rounded-2xl flex items-start gap-4 shadow-xs text-rose-950">
            ...
            <div className="space-y-1 text-xs">
              <h3 className="text-sm font-bold text-rose-900">
                Onboarding Case Rejected & Permissions Revoked
              </h3>
              <p className="font-medium">
                ...
                <em>
                  ...
                  {selectedCase.rejectionReason ||
                    "Compliance requirements not fulfilled."}
                  "
                </em>
              </p>
              <p className="text-rose-700">
                Recorded by {selectedCase.rejectedBy || currentUser.name} on{" "}
                {selectedCase.rejectedAt
                  ? new Date(selectedCase.rejectedAt).toLocaleDateString()
                  : "Today"}
              </p>
            </div>
          </div>
        ...
        <div className="p-6 bg-white rounded-2xl border border-slate-200 shadow-xs flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex items-start sm:items-center gap-5">
            <Avatar
              src={selectedCase.avatarUrl}
              name={selectedCase.candidateName}
              size="xl"
              className="shrink-0 ring-4 ring-slate-100"
            />
            <div>
              ...
              <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 mt-2 text-xs text-slate-500">
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  {selectedCase.phone || "Phone pending"}
                </span>
                <span className="flex items-center gap-1.5">
                ...
                <span className="flex items-center gap-1.5">
                  <Calendar className="w-3.5 h-3.5 text-slate-400" /> Joining
                  Date:{" "}
                  <strong className="text-slate-800">
                    {selectedCase.joiningDate}
                  </strong>
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="shadow-xs border border-slate-200">
              <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-slate-100">
                ...
                <Badge
                  variant={
                    selectedCase.offerSignedUploaded ? "emerald" : "amber"
                  }
                >
                  {selectedCase.offerSignedUploaded
                    ? "SIGNED COPY ATTACHED"
                    : "PENDING CANDIDATE UPLOAD"}
                </Badge>
              </CardHeader>
                ...
                {selectedCase.offerSignedUploaded ? (
                    ...
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-slate-900">
                            {selectedCase.offerSignedFileName ||
                              `Signed_Offer_${selectedCase.candidateName.replace(" ", "_")}.pdf`}
                          </h4>
                          <p className="text-xs text-slate-500">
                            Executed Digital e-Signature • Signed on{" "}
                            {selectedCase.offerSignedAt
                              ? new Date(
                                  selectedCase.offerSignedAt,
                                ).toLocaleString()
                              : "Recently"}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Button
                          ...
                          onClick={() =>
                            toast.success(
                              "Signed offer document downloaded successfully",
                            )
                          }
                  ...
                  <div className="p-6 bg-slate-50 rounded-xl border border-dashed border-slate-300 text-center text-xs text-slate-500">
                    ...
                    <p className="text-slate-400 mt-0.5">
                      The candidate has not yet signed their offer letter on the
                      digital canvas.
                    </p>
                  </div>
                )}
            ...
            <Card className="shadow-xs border border-slate-200">
              <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-slate-100">
                ...
                <Badge
                  variant={
                    selectedCase.requiredDocsUploaded ? "emerald" : "amber"
                  }
                >
                  {selectedCase.requiredDocsUploaded
                    ? "DOCUMENTS SUBMITTED"
                    : "INCOMPLETE / PENDING"}
                </Badge>
              </CardHeader>
              <CardContent className="pt-4 space-y-3">
                {selectedCase.uploadedDocs &&
                selectedCase.uploadedDocs.length > 0 ? (
                  ...
                  <div className="p-6 bg-slate-50 rounded-xl border border-dashed border-slate-300 text-center text-xs text-slate-500">
                    ...
                    <p className="text-slate-400 mt-0.5">
                      Government ID, Form W-4/16, and Educational certificates
                      pending candidate upload.
                    </p>
                  </div>
                )}
            ...
            <Card className="shadow-xs border border-slate-200">
              <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-slate-100">
                ...
                <Badge
                  variant={
                    selectedCase.acknowledgementSigned ? "emerald" : "amber"
                  }
                >
                ...
                {selectedCase.acknowledgementSigned ? (
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-700 space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white p-3 rounded-lg border border-slate-200 font-medium">
                      <div>
                        ...
                        <span className="font-bold text-slate-900">
                          {selectedCase.acknowledgementName ||
                            selectedCase.candidateName}
                        </span>
                      </div>
                      <div>
                        ...
                        <span className="font-bold text-slate-900">
                          {selectedCase.acknowledgementPlace ||
                      ...
                      <div>
                        ...
                        <span className="font-bold text-slate-900">
                          {selectedCase.acknowledgementDate ||
                            selectedCase.joiningDate}
                        </span>
                      </div>
                    </div>
                    ...
                    <p className="text-slate-600 leading-relaxed flex items-center gap-2">
                      ...
                      <span>
                        Verified electronic consent to Peopleworkplaces Code of
                        Conduct, Anti-Harassment Guidelines, and InfoSec
                        policies.
                      </span>
                    </p>
                  </div>
                  ...
                  <div className="p-6 bg-slate-50 rounded-xl border border-dashed border-slate-300 text-center text-xs text-slate-500">
                    ...
                    <p className="font-semibold text-slate-700">
                      Awaiting candidate policy signature
                    </p>
                    <p className="text-slate-400 mt-0.5">
                      The candidate has not yet submitted electronic policy
                      consent.
                    </p>
                  </div>
                )}
          ...
          <div className="space-y-6">
            ...
            <Card className="shadow-xs border border-slate-200">
              <CardHeader className="pb-3 border-b border-slate-100">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <Briefcase className="w-4 h-4 text-indigo-600" /> Official Job
                  Parameters
                </CardTitle>
              </CardHeader>
            ...
            <Card className="shadow-xs border border-slate-200">
              <CardHeader className="pb-3 border-b border-slate-100">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <UserCheck className="w-4 h-4 text-emerald-600" /> Candidate
              ...
              <CardContent className="pt-4 space-y-3 text-xs">
                <div>
                  ...
                  <span className="font-semibold text-slate-800">
                    {selectedCase.phone || "Not submitted yet"}
                  </span>
                </div>
                <div>
                  <span className="text-slate-400 font-medium block">
                  ...
                  <span className="font-semibold text-slate-800">
                    {selectedCase.address || "Not submitted yet"}
                  </span>
                </div>
                <div>
                  <span className="text-slate-400 font-medium block">
                  ...
                  <span className="font-semibold text-slate-800">
                    {selectedCase.emergencyContact || "Not submitted yet"}
                  </span>
                </div>
                <div>
                  <span className="text-slate-400 font-medium block">
                  ...
                  {selectedCase.funFact ? (
                    ...
                    <p className="text-slate-400 italic mt-0.5">
                      Awaiting candidate introduction note...
                    </p>
                  )}
                </div>
              </CardContent>
            ...
            <Card className="shadow-md border-2 border-emerald-500 bg-emerald-50/20 p-5 space-y-4">
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                Approving this case will instantly convert the candidate's
                account into a permanent active{" "}
                <strong className="text-slate-900">EMPLOYEE</strong> with full
            ...
            <Card className="max-w-md w-full p-6 space-y-4 bg-white shadow-2xl rounded-2xl border border-rose-200">
              <div className="flex items-center gap-3 text-rose-700">
                ...
                <div>
                  <h3 className="font-bold text-base text-slate-900">
                    Reject Onboarding Case
                  </h3>
                  <p className="text-xs text-slate-500">
                    Revoke permissions for {selectedCase.candidateName}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700">
                  Rejection Reason / Note to Candidate:
                </label>
                <textarea
                  value={rejectReason}
                ...
                <p className="text-[11px] text-slate-400">
                  This note will be shown on the candidate's onboarding screen
                  instructing them to contact HR.
                </p>
              </div>
              ...
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded border border-emerald-200">
                    <ShieldCheck className="w-3.5 h-3.5" /> SECURE COMPLIANCE
                    RECORD
                  </span>
                </div>
                  ...
                  <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm space-y-6 max-w-2xl mx-auto">
                    ...
                    <div className="p-4 bg-indigo-50/60 rounded-xl border border-indigo-100 text-xs text-indigo-900 leading-relaxed">
                      ...
                      <p className="text-slate-600">
                        This document has been safely encrypted, anti-malware
                        scanned, and linked to candidate onboarding case for
                        verification.
                      </p>
                    </div>
                  </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Track new hire task submissions, inspect uploaded documents, and
            approve or decline permanent employment access.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={() => {
              toast.info(
                "New onboarding invitations can be initiated from the Employees tab",
              );
            }}
          >
            + Invite New Hire
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="border-l-4 border-l-indigo-600">
          ...
          <p className="text-xs text-slate-500 mt-3">
            Pipeline candidates for {currentTenant.name}
          </p>
        </Card>
        ...
        <Card className="border-l-4 border-l-amber-500">
          ...
          <p className="text-xs text-slate-500 mt-3 font-medium text-amber-700">
            {readyForReview > 0
              ? "Action required: Awaiting HR sign-off"
              : "All submitted cases reviewed"}
          </p>
        </Card>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-2xs">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto">
          ...
          <Button
            ...
            className={
              readyForReview > 0 ? "border-amber-400 font-semibold" : ""
            }
          >
                    ...
                    <div>
                      <div className="flex items-center gap-2.5">
                        ...
                        {isRejected && (
                          <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-rose-100 text-rose-800 border border-rose-300 flex items-center gap-1">
                            <XCircle className="w-3 h-3" /> Access Revoked /
                            Rejected
                          </span>
                        )}
                      ...
                      <div className="flex items-center gap-3 mt-2 text-xs text-slate-500">
                        <span className="flex items-center gap-1">
                          ...
                          Start Date: <strong>{c.joiningDate}</strong>
                        </span>
                        <span>•</span>
                        <span>
                          Location: <strong>{c.regionName}</strong>
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm space-y-6 max-w-2xl mx-auto">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div className="p-3 bg-slate-50 rounded-lg">
                      ...
                      <strong className="text-slate-800">
                        {previewDoc.candidateName || "Candidate"}
                      </strong>
                    </div>
                    <div className="p-3 bg-slate-50 rounded-lg">
                      <span className="text-slate-400 block text-[10px] uppercase font-semibold">
