...
import { Download, CheckCircle2, ShieldCheck, FileCheck, AlertOctagon, Lock, ArrowRight } from 'lucide-react';
...
export const AcknowledgementPage: React.FC = () => {
  ...
  const isPhase3Locked = !myCase?.personalDetailsCompleted || !myCase?.offerSignedUploaded || !myCase?.requiredDocsUploaded;

  const [name, setName] = useState(myCase?.acknowledgementName || currentUser.name);
  const [place, setPlace] = useState(myCase?.acknowledgementPlace || '');
  const [date, setDate] = useState(myCase?.acknowledgementDate || new Date().toISOString().split('T')[0]);
  const [confirmed, setConfirmed] = useState(myCase?.acknowledgementSigned || false);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!confirmed) {
      toast.error('Please check the confirmation box to agree to company policies');
      return;
    }
    ...
    mockStorage.addAuditLog('ONBOARDING_ACKNOWLEDGEMENT_SIGNED', 'ONBOARDING_CASE', myCase?.id || currentUser.id);
    toast.success('🎉 Policy signed! All onboarding tasks submitted for HR review.');
    ...
    return (
      <div className="space-y-6 max-w-2xl mx-auto animate-in fade-in duration-200 text-center py-12">
        <div className="p-8 bg-white rounded-2xl border border-slate-200 shadow-sm space-y-4">
          ...
          <h2 className="text-xl font-bold text-slate-900">Phase 4 is Locked</h2>
          <p className="text-sm text-slate-500 max-w-md mx-auto">
            You must complete and submit <strong>Phase 3: Required Documents Submission</strong> before signing company policy acknowledgements.
          </p>
          <div className="pt-2">
            <Button
    ...
    <div className="space-y-6 w-full animate-in fade-in duration-200">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Review company policies and InfoSec rules, execute electronic sign-off with Name, Place & Date, and download your copy.
        </p>
      </div>
        ...
        <Card className="shadow-xs">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <ShieldCheck className="w-5 h-5 text-indigo-600" /> Formal Policy Sign-Off & Consent
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-700 leading-relaxed max-h-48 overflow-y-auto space-y-2">
              <p className="font-bold text-slate-900">Standard Code of Conduct, Anti-Harassment & InfoSec Agreement</p>
              <p>
                I hereby acknowledge that I have received, read, and understood the Cyrcalur HRMS and {currentTenant.name} Code of Conduct, Anti-Harassment Guidelines, Acceptable Asset Use, and Information Security Policies.
              </p>
              <p>
                I agree to adhere strictly to all organizational security procedures, handle company confidential data responsibly, and comply with applicable workplace regulations.
              </p>
            </div>

            <FormField label="Full Legal Name" required helperText="As stated on legal government identification">
              <Input
                value={name}
            ...
            <div className="p-3 bg-indigo-50/60 rounded-xl border border-indigo-100">
              <label className="flex items-start gap-2.5 cursor-pointer text-xs font-semibold text-slate-900">
                ...
                />
                <span>I explicitly confirm, agree to all terms stated above, and submit my onboarding case for HR review.</span>
              </label>
            </div>
          </CardContent>
          ...
          <CardFooter className="flex items-center justify-between bg-slate-50/50 rounded-b-xl border-t border-slate-100">
            ...
            <Button type="submit" disabled={isRejected} leftIcon={<FileCheck className="w-4 h-4" />}>
